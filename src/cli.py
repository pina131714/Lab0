...
def _parse_json_input(ctx, param, value: str) -> list:
    ...
    try:
        ...
        raise click.BadParameter(
            "Input must be a valid JSON string (e.g., '[[1,2],[3,4]]')."
        )
...
@click.argument("values", callback=_parse_list_input)
@click.option(
    "--min", "min_val", default=0.0, type=float, help="Minimum value to clip to."
)
@click.option(
    "--max", "max_val", default=1.0, type=float, help="Maximum value to clip to."
)
...
def flatten(values):
    ...
    if not isinstance(values, list) or (
        values and not all(isinstance(i, list) for i in values)
    ):
        click.echo(
            f"Error: Input must be a list of lists. e.g., '[[1,2],[3,4]]'", err=True
        )
        return
